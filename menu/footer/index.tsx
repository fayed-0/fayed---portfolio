...
const navItems = ['Home', 'Project', 'About', 'Certificate', 'Contact', 'Download CV']
...
export default function Footer() {
	return (
		<footer id="contact" className="w-full border-t border-zinc-200 dark:border-zinc-800">
			{/* ===== MOBILE (stacked, center) ===== */}
			<div className="flex md:hidden flex-col items-center gap-6 px-[10px] pt-10 pb-8">
				{/* Logo */}
				<Link href="/" aria-label="Home">
					<Image src="/logo.png" alt="Logo" width={56} height={56} className="dark:invert" />
				</Link>
				...
				<Link
					href="/#terms"
					className="text-zinc-400 dark:text-zinc-500 text-xs uppercase tracking-wide hover:opacity-70 transition"
				>
					Terms of Service
				</Link>

				<p className="text-zinc-400 dark:text-zinc-500 text-xs uppercase tracking-wide">
					© {new Date().getFullYear()} FayedAbdulHakim
				</p>
			</div>

			{/* ===== DESKTOP (single line) ===== */}
			<div className="hidden md:flex mx-[10px] lg:mx-[60px] pt-3.5 pb-2.5 flex-row justify-between items-center gap-2.5">
				<div className="flex flex-wrap items-center gap-3.5">
					<span className="text-zinc-600 dark:text-zinc-300 text-base font-bold uppercase leading-4 tracking-wide">
						Portfolio
					</span>
					{navItems.map((item) => (
						<Link
							key={item}
							href={linkMap[item] ?? '#'}
							className="text-zinc-600 dark:text-zinc-400 text-[10px] font-medium uppercase leading-4 tracking-wide hover:opacity-70 transition"
						>
							{item}
						</Link>
					))}
				...
				<div className="flex items-center gap-1.5">
					{socials.slice(0, 2).map((s) => (
						<a
							...
							target="_blank"
							rel="noopener noreferrer"
							className="w-5 h-5 grid place-items-center text-zinc-600 dark:text-zinc-400 hover:opacity-70 transition"
						>
							{s.icon}
						</a>
					))}
					<span className="text-zinc-600 dark:text-zinc-400 text-[10px] font-medium uppercase leading-4 tracking-wide whitespace-nowrap ml-1">
						© {new Date().getFullYear()} FayedAbdulHakim
					</span>
				</div>
			</div>
